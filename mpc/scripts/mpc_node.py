...
class MPC(Node):
    ...
    def __init__(self):
        ...
        with open('/media/karthik/DATA/sem2/f1tenth/lab1_ws/src/lab-8-model-predictive-controlc-team-5/mpc/waypoints.csv', newline='') as csvfile:
            data = np.array([list(map(float, row)) for row in csv.reader(csvfile)])

        self.waypoints = data[::16,:]
